    ...
    methods (Static = true)
            ...
            for i=1:numel(experiments)
                if ~(any(strcmp(experiments(i).name, {'.', '..'}))) && experiments(i).isdir
                    ...
                    for j=1:numel(predicted_files)
                        ...
                        %                      Add on TFG                      %
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        actual = TFGFileReadClass().ReadFile(datasetPath,real_files(j).name,0);
                        act{j} = actual.targets;
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function logsDir = configureExperiment(expFile,dirSuffix)
            ...
            for e = 1:num_experiment
                ...
                Utilities.checkDatasets(directory, datasets, archive); % Modificado TFG a�adiendo el parametro archive
                
                [train, test] = Utilities.processDirectory(directory,datasetsList,archive); % Modificado TFG a�adiendo el parametro archive
                ...
                for i=1:numel(train)
                    ...
                    fich = fopen(file,'w');
                    fprintf(fich, [directory '/' datasetsList{i} '/' archive]); % Modificado TFG cambiando 'matlab' por archive
                    fclose(fich);
                    ...
                    for j=1:runfolds
                        ...
                        expObj.general('directory') = [directory '/' datasetsList{i} '/' archive]; % Modificado TFG cambiando 'matlab' por archive
    ...
    methods(Static = true, Access = private)
        ...
        function [trainFileNames, testFileNames] = processDirectory(directory, dataSetNames, archive)
            ...
            for j=1:numel(dataSetNames)
                ...
                if(isdir(dsdirectory))
                    ...
                    %                      Add on TFG                      %
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    [trainFileNames{validDataSets},testFileNames{validDataSets}] = ...
                        TFGFileReadClass().TFGFileName(dsdirectory, archive, dataSetNames{j});
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function checkDatasets(basedir, datasets, archive)
            ...
            for i=1:length(dsdirsCell) % skip . and ..
                ...
                %                      Add on TFG                      %
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                datasetPath = [basedir '/' dsName '/' archive];
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                for f=1:length(dsTrainFiles)
                    ...
                    %try
                        ...
                        %                      Add on TFG                      %
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        cellData = TFGFileReadClass().ReadFile(datasetPath,trainName,0);
                        trainData = [cellData.patterns cellData.targets];
                        cellData = TFGFileReadClass().ReadFile(datasetPath,testName,0);
                        testData = [cellData.patterns cellData.targets];
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    %catch
